...
function three_household_fixed_bubble_visit_plan!(hh_visit_plan::Array{Array{Array{Int64,1},1},2},
    ...
    for first_hh_in_bubble_ID = 1:n_hh
        ...
        # # Update eligible households to be selected.
        # # If zero, then just continue.
        # hh_not_in_support_bubble_with_current_size_zero_xmas_bubble = hh_not_in_support_bubble[length.(hh_mixed_with_by_day[hh_not_in_support_bubble]) .== 0]
        ...
        # hh_with_space_in_xmas_bubble = hh_ID_vec[length.(hh_mixed_with_by_day) .< 2]
...
#                                         rng::MersenneTwister)
#
#     # Allocate households into groups of three
#
#     # Begin with getting list of households IDs
#     n_households = length(info_per_household)
#     households_IDs = collect(1:n_households)
#
#     # Shuffle order and group into triples
#     shuffle!(rng,households_IDs)
#     n_triples = ceil(Int64,n_households/3)
#
#     #Initialise households visited per timestep for each person
#     mixing_start_time = 1 + pre_christmas_bubble_time
#     mixing_end_time = 5 + pre_christmas_bubble_time
#     hh_mixed_with_by_day = Array{Array{Int64,1},1}(undef,n_households)
#
#     # Assign households contacted on each day
#     for hh_grp_itr = 1:n_triples
#         start_idx = ((hh_grp_itr-1)*3) + 1
#         end_idx = min(hh_grp_itr*3,n_households)
#         household_ID_slice = households_IDs[start_idx:end_idx]
#         for person_itr = 1:length(household_ID_slice)
#             index_person_ID = household_ID_slice[person_itr]
#             hh_mixed_with_by_day[index_person_ID] = []
#             for contact_itr = 1:length(household_ID_slice)
#                 if person_itr != contact_itr
#                     contact_person_ID = household_ID_slice[contact_itr]
#                     append!(hh_mixed_with_by_day[index_person_ID],contact_person_ID)
#                 end
#             end
#         end
#     end
#
#     # Placeholder. Check if in support bubble.
#     # Populate each timestep with support bubble household ID
#     for person_itr = 1:n_persons
#         hh_ID::Int64 = info_per_person[person_itr].household_ID
#
...
#                     support_bubble_hh_ID::Int64 = info_per_household[hh_ID].support_bubble_household_ID
#
#                     # Assign households mixed with on given day
#                     if !isempty(hh_mixed_with_by_day[hh_ID])
#                         hh_visit_plan[timestep_itr,person_itr] = [vcat(support_bubble_hh_ID,hh_mixed_with_by_day[hh_ID])]
#                     else
#                         hh_visit_plan[timestep_itr,person_itr] = [[support_bubble_hh_ID]]
#                     end
#                 else
...
#         household_ID_slice = households_IDs[start_idx:end_idx]
#         for person_itr = 1:length(household_ID_slice)
#             index_hh_ID = household_ID_slice[person_itr]
#             hh_mixed_with_by_day[index_hh_ID] = []
#             for contact_itr = 1:length(household_ID_slice)
#                 if person_itr != contact_itr
#                     contact_hh_ID = household_ID_slice[contact_itr]
#                     append!(hh_mixed_with_by_day[index_hh_ID],contact_hh_ID)
#                 end
#             end
#         end
...
#
#     # Assign households contacted on each day
#     for hh_grp_itr = 1:n_households
#         hh_mixed_with_by_day[hh_grp_itr] = []
#         bubble_household_IDs = zeros(Int64,2)
#         for bubble_hh_itr = 1:length(bubble_household_IDs)
#             sampled_hh_ID = rand(rng,1:n_households)
#
#             #If second iteration, redraw if matches draw from first iteration
#             second_itr_check_flag = false
#             if bubble_hh_itr == 2
#                 while second_itr_check_flag == false
#                     if sampled_hh_ID == bubble_household_IDs[1]
#                         sampled_hh_ID = rand(rng,1:n_households)
#                     else
#                         second_itr_check_flag = true
#                     end
#                 end
#             end
#
#             # Redraw if bubble household selected is the same as the index house or is already in bubble with them
#             hh_check_flag = false
#             if (sampled_hh_ID == hh_grp_itr) ||
#                 (info_per_household[hh_grp_itr].support_bubble_household_ID == hh_grp_itr)
#                 sampled_hh_ID = rand(rng,1:n_households)
#             else
#                 hh_check_flag = true
#             end
#
#             # Assign household drawn to vector
#             bubble_household_IDs[bubble_hh_itr] = sampled_hh_ID
#         end
#         # Assign to ID storage vector
#         hh_mixed_with_by_day[hh_grp_itr] = 1*bubble_household_IDs
#     end
#
#     # Placeholder. Check if in support bubble.
#     # Populate each timestep with support bubble household ID
...
#     mixing_start_time = 1 + pre_christmas_bubble_time
#     mixing_end_time = 5 + pre_christmas_bubble_time
#     n_mixing_days = mixing_end_time - mixing_start_time + 1
#     hh_mixed_with_by_day = Array{Array{Int64,1},2}(undef,n_households,n_mixing_days)
#     for timestep_itr = 1:n_mixing_days
#         # Shuffle order and group into triples
#         shuffle!(rng,households_IDs)
#         n_triples = ceil(Int64,n_households/3)
#
...
#             for person_itr = 1:length(household_ID_slice)
#                 index_person_ID = household_ID_slice[person_itr]
#                 hh_mixed_with_by_day[index_person_ID,timestep_itr] = []
#                 for contact_itr = 1:length(household_ID_slice)
#                     if person_itr != contact_itr
#                         contact_person_ID = household_ID_slice[contact_itr]
#                         append!(hh_mixed_with_by_day[index_person_ID,timestep_itr],contact_person_ID)
#                     end
#                 end
#             end
#         end
...
#         for timestep_itr = 1:endtime
#             if mixing_start_time <= timestep_itr <= mixing_end_time
#                 mixing_day_idx = (timestep_itr - mixing_start_time) + 1
#                 if info_per_household[hh_ID].in_support_bubble == true
#                     # ID of other household in the support bubble (if index household is in one)
#                     support_bubble_hh_ID::Int64 = info_per_household[hh_ID].support_bubble_household_ID
#
#                     # Assign households mixed with on given day
#                     if !isempty(hh_mixed_with_by_day[hh_ID,mixing_day_idx])
#                         hh_visit_plan[timestep_itr,person_itr] = [vcat(support_bubble_hh_ID,hh_mixed_with_by_day[hh_ID,mixing_day_idx])]
#                     else
#                         hh_visit_plan[timestep_itr,person_itr] = [[support_bubble_hh_ID]]
#                     end
#                 else
#                     if !isempty(hh_mixed_with_by_day[hh_ID,mixing_day_idx])
#                         hh_visit_plan[timestep_itr,person_itr] = [hh_mixed_with_by_day[hh_ID,mixing_day_idx]]
#                     else
#                         hh_visit_plan[timestep_itr,person_itr] = [[]]
#                     end
#                 end
